...
#include "esp_netif.h" //changed from tcpip
...
#define UART_GPIO_RTS CONFIG_GSM_DTS
#define UART_GPIO_CTS CONFIG_GSM_CTS
#define UART_GPIO_PWR CONFIG_GSM_PWR
#define UART_GPIO_DTR CONFIG_GSM_DTR
...
static GSM_Cmd cmd_AutoBDrate =
{
	.cmd = "AT+IPR=0\r\n",
	.cmdSize = sizeof("AT+IPR=0\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 300,
	.delayMs = 0,
	.skip = 0,
...
//static GSM_Cmd cmd_FlwCtr =
//{
//	.cmd = "AT+IFC=2,2\r\n",
//	.cmdSize = sizeof("AT+IFC=2,2\r\n")-1,
//	.cmdResponseOnOk = GSM_OK_Str,
//	.timeoutMs = 300,
//	.delayMs = 0,
//	.skip = 0,
...
static GSM_Cmd cmd_Cops =
{
	...
	.cmdResponseOnOk = "AT&T Twilio",
	.timeoutMs = 300,
	.delayMs = 0,
	.skip = 0,
};

static GSM_Cmd cmd_LTE =
{
	.cmd = "AT+CNMP=38\r\n",
	.cmdSize = sizeof("AT+CNMP=38\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 300,
	.delayMs = 0,
	.skip = 0,
...
static GSM_Cmd cmd_Band =
{
	.cmd = "AT+CBAND=EGSM_MODE\r\n",
	.cmdSize = sizeof("AT+CBAND=EGSM_MODE\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 1000,
	.delayMs = 0,
	.skip = 0,
...
static GSM_Cmd cmd_Cat =
{
	.cmd = "AT+CMNB=1\r\n",
	.cmdSize = sizeof("AT+CMNB=1\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 300,
	.delayMs = 0,
	.skip = 0,
...
static GSM_Cmd cmd_GPSon = 
{
	.cmd = "AT+SGPIO=0,4,1,1\r\n",
	.cmdSize = sizeof("AT+SGPIO=0,4,1,1\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 3000,
	.delayMs = 1000,
	.skip = 0,
...
static GSM_Cmd cmd_GPSpoweron = 
{
	.cmd = "AT+CGNSPWR=1\r\n",
	.cmdSize = sizeof("AT+CGNSPWR=1\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 3000,
	.delayMs = 1000,
	.skip = 0,
...
static GSM_Cmd cmd_GPSpoweroff = 
{
	.cmd = "AT+CGNSPWR=0\r\n",
	.cmdSize = sizeof("AT+CGNSPWR=0\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 3000,
	.delayMs = 1000,
	.skip = 0,
...
static GSM_Cmd cmd_GPSoutput = 
{
	.cmd = "AT+CGNSINF\r\n",
	.cmdSize = sizeof("AT+CGNSINF\r\n")-1,
	.cmdResponseOnOk = "1,1",
	.timeoutMs = 30000,
	.delayMs = 1000,
	.skip = 0,
...
static GSM_Cmd cmd_GPSoff = 
{
	.cmd = "AT+SGPIO=0,4,1,0\r\n",
	.cmdSize = sizeof("AT+SGPIO=0,4,1,0\r\n")-1,
	.cmdResponseOnOk = GSM_OK_Str,
	.timeoutMs = 3000,
	.delayMs = 1000,
	.skip = 0,
...
static void infoCommand(char *cmd, int cmdSize, char *info)
{
	...
	ESP_LOGI(TAG,"%s [%s] [%d]", info, buf, cmdSize);
}
...
static int atCmd_waitResponse(char * cmd, char *resp, char * resp1, int cmdSize, int timeout, char **response, int size)
{
	...
	while(1)
	{
		...
		//ESP_LOGI(TAG,"uart length [%i]", len);
		if (len > 0) {
			for (int i=0; i<len;i++) {
				if (idx < 256) {
...
static void _disconnect(uint8_t rfOff)
{
	...
	//atCmd_waitResponse("AT+CSCLK=0\r\n", GSM_OK_Str, NULL, 12, 1000, NULL, 0);
	while (res == 0) {
		n++;
		if (n > 10) {
			#if GSM_DEBUG
...
static void pppos_client_task()
{
	...
	//if (uart_set_pin(uart_num, UART_GPIO_TX, UART_GPIO_RX, UART_GPIO_RTS, UART_GPIO_CTS)) goto exit;
...
void get_GPS()
{
	...
	ESP_LOGI(TAG,"Inside the gps method");
	...
	vTaskDelay(1000 / portTICK_PERIOD_MS);
	uart_flush(uart_num);
	ESP_LOGI(TAG,"uart is flushed, now writing to uart");
	uart_write_bytes(uart_num, "+++", 3);
	ESP_LOGI(TAG,"wrote to uart");
    uart_wait_tx_done(uart_num, 10 / portTICK_RATE_MS);
	vTaskDelay(1100 / portTICK_PERIOD_MS);
	ESP_LOGI(TAG,"In command mode");

	int res = atCmd_waitResponse("AT+SGPIO=0,4,1,1\r\n", GSM_OK_Str, NULL, -1, 3000, NULL, 0);
	if (res == 0)
		{
			ESP_LOGI(TAG,"FAILED TO TURN ON ANTENNA");
			return;
		}
	else
	{
		ESP_LOGI(TAG,"ANTENNA ON");
		res = atCmd_waitResponse("AT+CGNSPWR=1\r\n", GSM_OK_Str, NULL, -1, 300, NULL, 0);
		if (res == 0)
		{
			ESP_LOGI(TAG,"FAILED TO TURN ON GNSS");
			return;
		}
		else
		{
			res = atCmd_waitResponse("AT+CGNSINF\r\n", "1,1", NULL, -1, 30000, NULL, 0);

			int retry = 0;
			while (retry <= 25 && res == 0)
			{
				res = atCmd_waitResponse("AT+CGNSINF\r\n", "1,1", NULL, -1, 30000, NULL, 0);
				vTaskDelay(5000 / portTICK_PERIOD_MS);
				retry++;
			...
			{
				char *gpsptr;
				gpsptr = strstr(gpsbuffer, ".000,");
				if (gpsptr) ESP_LOGI(TAG, "string found: %s ", gpsptr);
				gpsptr += 5;
			
				char *tok;

				tok = strtok(gpsptr, ",");
				memcpy(lat, tok, strlen(tok)+1);
				tok = strtok(NULL, ",");
				memcpy(lon, tok, strlen(tok)+1);
				tok = strtok(NULL, ",");
				memcpy(alt, tok, strlen(tok)+1);
			}
		}
	}
	res = atCmd_waitResponse("AT+CGNSPWR=0\r\n", GSM_OK_Str, NULL, -1, 300, NULL, 0);            // ADD ERROR CHECK WHEN TURNING OFF ANTENNA
	res = atCmd_waitResponse("AT+SGPIO=0,4,1,0\r\n", GSM_OK_Str, NULL, -1, 300, NULL, 0);
